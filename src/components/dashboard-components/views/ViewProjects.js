...
import * as projectsRedux from "../../../redux/features/projects";
import Image from "next/image";

function ViewProjects() {
  const dispatch = useAppDispatch();
  const projectsState = useAppSelector(projectsRedux.reducer.selectProjects);
  const loadingState = useAppSelector(
    projectsRedux.reducer.selectProjectsLoadingState
  );

  const [reset, setReset] = useState(false);

  useEffect(() => {
    dispatch(projectsRedux.actions.fetchProjectsAsync());
  }, []);
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="bg-th-background  rounded-2xl">
          ...
          <div className="bg-neutral-400 text-sm flex flex-row items-center p-4 rounded-t-2xl gap-8">
            ...
            <div className="text-white w-1/4 flex-none">Stage</div>
...
export function AddModal({ isOpen, closeModal, data }) {
  ...
  const menuItems = ["Active", "Ideation"];
  ...
  function dispatchAddHandler() {
    dispatch(
      projectsRedux.actions.addNewProjectAndFetch(
        {
          name: name,
          stage: stage,
          sector: sector,
  ...
  function dispatchUpdateHandler() {
    dispatch(
      projectsRedux.actions.updateProjectAndFetch({
        id: id,
        name: name,
        stage: stage,
        sector: sector,
  ...
  function dispatchUpdateImageHandler() {
    dispatch(
      projectsRedux.actions.updateProjectImageAndFetch(
        {
          id: id,
          name: name,
          stage: stage,
  ...
  function dispatchDeleteHandler() {
    dispatch(projectsRedux.actions.deleteProjectAndFetch(id));
  }
